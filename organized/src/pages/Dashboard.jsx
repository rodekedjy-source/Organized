...
function AiCoach({ appts, stats, workspace }) {
  ...
  if(pending.length>0)
    insights.push(`You have ${pending.length} unconfirmed booking${pending.length>1?'s':''} waiting. Confirming ${pending.length>1?'them':'it'} secures your client's commitment.`)
  ...
  if(noReminder.length>0)
    insights.push(`${noReminder.length} client${noReminder.length>1?'s have':' has'} not received a reminder for today's appointment${noReminder.length>1?'s':''}. A message now reduces no-shows significantly.`)
  const goal = workspace?.monthly_revenue_goal||3000
  const mRev = monthRevenue(appts)
  ...
  if(pctG<50&&avg>0){
    ...
    insights.push(`You are at ${pctG}% of your ${new Date().toLocaleDateString('en-US',{month:'long'})} goal. ${needed} more appointment${needed>1?'s':''} at your current average would close the gap.`)
  } else if(pctG>=50&&pctG<90){
    insights.push(`You are at ${pctG}% of your monthly goal. Strong progress — keep your schedule filled.`)
  }
  ...
  if(upcoming.length===0&&insights.length<2)
    insights.push('Your next 3 days are open. Sharing your booking link today could fill those slots before the week ends.')
  if(insights.length===0) return null
  return (
    <div className="ai-coach">
      <div className="ai-coach-header">Coach</div>
      {insights.slice(0,3).map((tip,i)=>(
        <div key={i} className="ai-coach-row">
          <div className="ai-coach-line"/>
          <div className="ai-coach-text">{tip}</div>
        </div>
      ))}
    </div>
  )
      ...
      <div className="grid-2" style={{marginBottom:'1.25rem'}}>
        ...
        <div className="card" style={{marginBottom:0}}>
          <div className="card-head">
            ...
            <span style={{fontSize:'.72rem',color:'var(--ink-3)'}}>Tap a date to view or block</span>
          </div>
...
function Availability({ workspace, toast }) {
  ...
  return (
    <div>
      <div className="page-head"><div><div className="page-title">Availability</div><div className="page-sub">Set your schedule and block dates</div></div></div>
...
function Settings({ workspace, toast, refetch, theme, setTheme, session, ownerData }) {
  ...
  if(section==='profile') return (
    <div>
      <div className="page-head"><div><BackBtn/><div className="page-title" style={{marginTop:'.4rem'}}>My Profile</div></div></div>
      <div className="card" style={{marginBottom:'1.25rem'}}>
        <div className="card-head"><div className="card-title">Personal information</div></div>
        <SettingsProfileForm session={session} ownerData={ownerData} toast={toast} refetch={refetch}/>
      </div>
      <div className="card">
        <div className="card-head"><div className="card-title">Password</div></div>
        <SettingsPasswordForm session={session} toast={toast}/>
      </div>
    </div>
  )
  ...
  if(section==='business') return (
    <div>
      <div className="page-head">
        <div><BackBtn/><div className="page-title" style={{marginTop:'.4rem'}}>My Business</div></div>
        ...
        }}>{workspace?.is_published?'Unpublish':'Publish profile'}</button>
      </div>
      <SettingsBusinessForm workspace={workspace} toast={toast} refetch={refetch}/>
    </div>
  )

  if(section==='appearance') return (
    <div>
      <div className="page-head"><div><BackBtn/><div className="page-title" style={{marginTop:'.4rem'}}>Appearance</div></div></div>
      <div className="card">
  ...
  if(section==='language') return (
    <div>
      <div className="page-head"><div><BackBtn/><div className="page-title" style={{marginTop:'.4rem'}}>Language</div></div></div>
      <SettingsLanguageForm ownerData={ownerData} toast={toast} refetch={refetch}/>
    </div>
  )
...
function SettingsProfileForm({ session, ownerData, toast, refetch }) {
...
function SettingsPasswordForm({ session, toast }) {
  ...
  return (
    <form onSubmit={save} className="card-body" style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
      ...
      <button type="submit" className="btn btn-primary" style={{justifyContent:'center',padding:'.75rem'}} disabled={loading}>{loading?'Sending...':'Update password'}</button>
    </form>
  )
}

function SettingsBusinessForm({ workspace, toast, refetch }) {
...
function SettingsLanguageForm({ ownerData, toast, refetch }) {
  ...
  async function save(){
    setLoading(true);setSaved(false)
    if(ownerData?.id) await supabase.from('users').update({language:lang}).eq('id',ownerData.id)
    toast('Language saved.');setSaved(true);setLoading(false)
  }
  return (
    <div className="card">
      ...
      <div style={{padding:'1rem 1.4rem',display:'flex',flexDirection:'column',gap:'.5rem'}}>
        ...
        <button className="btn btn-primary" style={{justifyContent:'center',padding:'.75rem',marginTop:'.5rem'}} onClick={save} disabled={loading}>
          {loading?'Saving...':saved?'Saved':'Save language'}
        </button>
      </div>
    </div>
  )
...
export default function Dashboard({ session }) {
  ...
  const navItems=[
    ...
    {key:'availability', label:'Availability',  icon:I.avail},
...
.ai-coach-header{font-size:.68rem;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);margin-bottom:.65rem;}
...
.ai-coach-line{width:2px;min-height:32px;background:var(--border);border-radius:2px;flex-shrink:0;margin-top:.2rem;}
