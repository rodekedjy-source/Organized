...
function ProductDetailModal({ product, onClose }) {
  ...
  useEffect(()=>{
    const onKey = e => { if(e.key==='Escape') onClose(); if(e.key==='ArrowRight') setActiveIdx(i=>(i+1)%Math.max(images.length,1)); if(e.key==='ArrowLeft') setActiveIdx(i=>(i-1+Math.max(images.length,1))%Math.max(images.length,1)) }
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.72)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center',padding:'1rem',backdropFilter:'blur(4px)'}} onClick={onClose}>
      <div style={{background:'var(--surface)',borderRadius:18,width:'100%',maxWidth:680,maxHeight:'90vh',overflowY:'auto',boxShadow:'0 32px 80px rgba(0,0,0,.3)',display:'flex',flexDirection:'column'}} onClick={e=>e.stopPropagation()}>
        {/* Image viewer */}
        <div style={{position:'relative',background:'var(--bg)',borderRadius:'18px 18px 0 0',overflow:'hidden'}}>
          {images.length>0?(
            <>
              <div style={{height:320,display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',cursor:zoomed?'zoom-out':'zoom-in'}} onClick={()=>setZoomed(z=>!z)}>
                <img src={images[activeIdx]} alt="" style={{maxHeight:'100%',maxWidth:'100%',objectFit:'contain',transition:'transform .3s ease',transform:zoomed?'scale(1.85)':'scale(1)'}}/>
              </div>
              {images.length>1&&(
                <>
                  <button onClick={e=>{e.stopPropagation();setActiveIdx(i=>(i-1+images.length)%images.length)}}
                    style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',background:'rgba(0,0,0,.45)',border:'none',color:'#fff',width:36,height:36,borderRadius:'50%',cursor:'pointer',fontSize:'1.1rem',display:'flex',alignItems:'center',justifyContent:'center'}}>‹</button>
                  <button onClick={e=>{e.stopPropagation();setActiveIdx(i=>(i+1)%images.length)}}
                    style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',background:'rgba(0,0,0,.45)',border:'none',color:'#fff',width:36,height:36,borderRadius:'50%',cursor:'pointer',fontSize:'1.1rem',display:'flex',alignItems:'center',justifyContent:'center'}}>›</button>
                </>
              )}
              {zoomed&&<div style={{position:'absolute',bottom:10,left:'50%',transform:'translateX(-50%)',background:'rgba(0,0,0,.55)',color:'#fff',fontSize:'.7rem',padding:'3px 10px',borderRadius:20,pointerEvents:'none'}}>Click to zoom out</div>}
              {!zoomed&&<div style={{position:'absolute',bottom:10,left:'50%',transform:'translateX(-50%)',background:'rgba(0,0,0,.55)',color:'#fff',fontSize:'.7rem',padding:'3px 10px',borderRadius:20,pointerEvents:'none'}}>Click to zoom</div>}
            </>
          ):(
            <div style={{height:200,display:'flex',alignItems:'center',justifyContent:'center',color:'var(--ink-3)',fontSize:'.82rem'}}>No photos yet</div>
          )}
          <button onClick={onClose} style={{position:'absolute',top:12,right:12,background:'rgba(0,0,0,.45)',border:'none',color:'#fff',width:32,height:32,borderRadius:'50%',cursor:'pointer',fontSize:'.9rem',display:'flex',alignItems:'center',justifyContent:'center'}}>✕</button>
          {images.length>1&&(
            <div style={{position:'absolute',bottom:10,right:14,background:'rgba(0,0,0,.45)',color:'#fff',fontSize:'.7rem',padding:'3px 8px',borderRadius:20}}>{activeIdx+1}/{images.length}</div>
          )}
        </div>
        {/* Thumbnails */}
        {images.length>1&&(
          <div style={{display:'flex',gap:'.5rem',padding:'.75rem 1.25rem',overflowX:'auto',background:'var(--bg)',scrollbarWidth:'none'}}>
            {images.map((img,i)=>(
              <div key={i} onClick={()=>setActiveIdx(i)}
                style={{width:54,height:54,flexShrink:0,borderRadius:8,overflow:'hidden',border:`2px solid ${i===activeIdx?'var(--gold)':'transparent'}`,cursor:'pointer',transition:'border .14s'}}>
        ...
        <div style={{padding:'1.25rem 1.5rem'}}>
          <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'.75rem'}}>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:'1.45rem',color:'var(--ink)',fontWeight:500}}>{product.name}</div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:'1.4rem',color:'var(--gold)',flexShrink:0,marginLeft:'1rem'}}>{fmtRev(product.price)}</div>
          </div>
          {product.description&&<div style={{fontSize:'.85rem',color:'var(--ink-2)',lineHeight:1.65,marginBottom:'.85rem'}}>{product.description}</div>}
...
function Products({ workspace, toast }) {
  ...
  const [pendingImgs,setPendingImgs]=useState([]) // { file, preview, url }
  ...
  async function handleImageSelect(e){
    ...
    const previews=files.map(f=>({file:f,preview:URL.createObjectURL(f),url:null}))
    setPendingImgs(prev=>[...prev,...previews])
    // upload each to storage
    setUploadingImgs(true)
    const uploaded=await Promise.all(files.map(async(file,i)=>{
      const ext=file.name.split('.').pop()
      const path=`${workspace.id}/${Date.now()}-${i}.${ext}`
      const{error}=await supabase.storage.from('product-images').upload(path,file,{upsert:true})
      if(error) return null
      const{data:urlData}=supabase.storage.from('product-images').getPublicUrl(path)
      return urlData?.publicUrl||null
    }))
    setPendingImgs(prev=>prev.map((p,idx)=>{
      const match=uploaded[idx-(prev.length-files.length)]
      return match!==undefined?{...p,url:match}:p
    }))
    // simpler: just overwrite the last N entries
    setPendingImgs(prev=>{
      const before=prev.slice(0,prev.length-files.length)
      const updated=files.map((f,i)=>({file:f,preview:URL.createObjectURL(f),url:uploaded[i]}))
      return[...before,...updated]
    })
    setUploadingImgs(false)
  }

  function removeImg(i){ setPendingImgs(prev=>prev.filter((_,idx)=>idx!==i)) }

  async function add(e){
    e.preventDefault(); setSaving(true)
    const imageUrls=pendingImgs.map(p=>p.url).filter(Boolean)
      ...
      {showForm&&(
        <div className="card" style={{marginBottom:'1.25rem'}}>
          ...
          <form onSubmit={add} className="card-body" style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
            ...
            {/* Photo upload zone */}
            <div>
              <div style={{fontSize:'.78rem',fontWeight:600,color:'var(--ink-3)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'.6rem'}}>Photos</div>
              <label style={{display:'block',border:'2px dashed var(--border-2)',borderRadius:12,padding:'1.25rem',textAlign:'center',cursor:'pointer',transition:'border .15s,background .15s',background:'var(--bg)'}}
                ...
                <div style={{fontSize:'.82rem',fontWeight:500,color:'var(--ink-2)'}}>Click to upload photos</div>
                <div style={{fontSize:'.72rem',color:'var(--ink-3)',marginTop:'.2rem'}}>Multiple angles supported · JPG, PNG, WEBP</div>
              </label>
              {pendingImgs.length>0&&(
                <div style={{display:'flex',gap:'.5rem',flexWrap:'wrap',marginTop:'.75rem'}}>
                  {pendingImgs.map((img,i)=>(
                    <div key={i} style={{position:'relative',width:72,height:72,borderRadius:9,overflow:'hidden',border:'1px solid var(--border)',background:'var(--bg)'}}>
                      <img src={img.preview} alt="" style={{width:'100%',height:'100%',objectFit:'cover',opacity:img.url?1:.55,transition:'opacity .2s'}}/>
                      {!img.url&&<div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'.6rem',color:'var(--ink-3)'}}>↑</div>}
                      {img.url&&(
                        <button type="button" onClick={()=>removeImg(i)}
                          style={{position:'absolute',top:3,right:3,background:'rgba(0,0,0,.55)',border:'none',color:'#fff',width:18,height:18,borderRadius:'50%',cursor:'pointer',fontSize:'.6rem',display:'flex',alignItems:'center',justifyContent:'center'}}>✕</button>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <button type="submit" className="btn btn-primary" style={{justifyContent:'center',padding:'.75rem'}} disabled={saving||uploadingImgs}>
              {uploadingImgs?'Uploading photos…':saving?'Saving…':'Save product'}
            </button>
          </form>
        </div>
      )}
      ...
      <div className="grid-3">
        {data.length===0
          ...
          :data.map(p=>{
            const imgs=p.images||[]
            return (
              <div key={p.id} className="prod-card" onClick={()=>setSelectedProduct(p)} style={{cursor:'pointer'}}>
                <div className="prod-img" style={{position:'relative',overflow:'hidden',background:'var(--bg)'}}>
                  {imgs.length>0
                    ?<img src={imgs[0]} alt={p.name} style={{width:'100%',height:'100%',objectFit:'cover',transition:'transform .3s ease'}}
                        onMouseEnter={e=>e.currentTarget.style.transform='scale(1.06)'}
                        onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}/>
                    :<div style={{width:'100%',height:'100%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'.7rem',color:'var(--ink-3)',letterSpacing:'.06em',textTransform:'uppercase'}}>PRODUCT</div>
                  }
                  {imgs.length>1&&(
                    <div style={{position:'absolute',bottom:6,right:7,background:'rgba(0,0,0,.5)',color:'#fff',fontSize:'.62rem',padding:'2px 7px',borderRadius:20}}>+{imgs.length-1}</div>
                  )}
                </div>
...
function fileType(f){ if(f.type?.startsWith('video')) return 'video'; if(f.type?.startsWith('image')) return 'image'; if(f.type==='application/pdf') return 'pdf'; return 'other' }
...
function Formations({ workspace, toast }) {
  ...
  const [pendingFiles,setPendingFiles]=useState([]) // {file,preview?,url,name,size,kind}
  ...
  async function handleFiles(e){
    ...
    const results=await Promise.all(files.map(async file=>{
      ...
      const{error}=await supabase.storage.from('formation-files').upload(path,file,{upsert:true})
      if(error) return null
      const{data:urlData}=supabase.storage.from('formation-files').getPublicUrl(path)
      return{name:file.name,size:file.size,kind,url:urlData?.publicUrl||null,preview:kind==='image'?URL.createObjectURL(file):null}
    }))
    setPendingFiles(prev=>[...prev,...results.filter(Boolean)])
    setUploading(false)
  }

  async function add(e){
    e.preventDefault(); setSaving(true)
    const filesData=pendingFiles.map(({name,size,kind,url})=>({name,size,kind,url}))
    ...
    <div>
      ...
      {showForm&&(
        <div className="card" style={{marginBottom:'1.25rem'}}>
          <div className="card-head">
            <div>
              <div className="card-title">New formation</div>
              <div className="card-sub">Share your knowledge. Set your price. Own your revenue.</div>
            </div>
          </div>
          <form onSubmit={add} className="card-body" style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
            ...
            <div>
              ...
              <label style={{display:'block',border:'2px dashed var(--border-2)',borderRadius:12,padding:'1.25rem',textAlign:'center',cursor:'pointer',transition:'all .15s',background:'var(--bg)'}}
                onMouseEnter={e=>e.currentTarget.style.borderColor='var(--gold)'}
                onMouseLeave={e=>e.currentTarget.style.borderColor='var(--border-2)'}>
                <input type="file" accept="image/*,video/*,.pdf" multiple style={{display:'none'}} onChange={handleFiles}/>
                <div style={{fontSize:'1.5rem',marginBottom:'.35rem'}}>📚</div>
                <div style={{fontSize:'.82rem',fontWeight:500,color:'var(--ink-2)'}}>Upload course materials</div>
                <div style={{fontSize:'.72rem',color:'var(--ink-3)',marginTop:'.2rem'}}>PDF guides, tutorial photos, intro videos</div>
              </label>
              {uploading&&<div style={{fontSize:'.78rem',color:'var(--ink-3)',padding:'.5rem 0',textAlign:'center'}}>Uploading files…</div>}
              {pendingFiles.length>0&&(
                <div style={{display:'flex',flexDirection:'column',gap:'.4rem',marginTop:'.65rem'}}>
                  {pendingFiles.map((f,i)=>(
                    <div key={i} style={{display:'flex',alignItems:'center',gap:'.65rem',padding:'.55rem .85rem',background:'var(--bg)',borderRadius:9,border:'1px solid var(--border)'}}>
                      {f.preview?<img src={f.preview} alt="" style={{width:38,height:38,objectFit:'cover',borderRadius:6,flexShrink:0}}/>
                        :<span style={{fontSize:'1.2rem',flexShrink:0}}>{FILE_ICONS[f.kind]||FILE_ICONS.other}</span>}
                      <div style={{flex:1,overflow:'hidden'}}>
                        <div style={{fontSize:'.8rem',fontWeight:500,color:'var(--ink)',textOverflow:'ellipsis',overflow:'hidden',whiteSpace:'nowrap'}}>{f.name}</div>
                        <div style={{fontSize:'.7rem',color:'var(--ink-3)'}}>{fmtBytes(f.size)}</div>
                      </div>
            ...
            <button type="submit" className="btn btn-primary" style={{justifyContent:'center',padding:'.75rem'}} disabled={saving||uploading}>
              {uploading?'Uploading…':saving?'Saving…':'Create formation'}
            </button>
          </form>
        </div>
      )}
                    ...
                    <div style={{display:'flex',gap:'.25rem'}}>
                      {(f.files||[]).slice(0,4).map((file,fi)=>(
                        <a key={fi} href={file.url} target="_blank" rel="noopener noreferrer"
                          ...
                          {FILE_ICONS[file.kind]||FILE_ICONS.other} {file.name.length>12?file.name.slice(0,12)+'…':file.name}
                        </a>
                      ))}
