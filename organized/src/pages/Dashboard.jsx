...
function Overview({ workspace, toast }) {
  ...
  useEffect(() => {
    ...
    // Realtime — overview stats update instantly when new booking arrives
        ...
        <div className="db-card" style={{marginBottom:'1.25rem'}}>
          ...
          <form onSubmit={add} style={{padding:'1.4rem',display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem'}}>
            <div className="db-field" style={{gridColumn:'1/-1'}}>
              ...
              <input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} placeholder="e.g. Box Braids" required/>
            </div>
            <div className="db-field">
              <label>Price (CAD) — enter 0 for free</label>
              <input type="number" value={form.price} onChange={e=>setForm(f=>({...f,price:e.target.value}))} placeholder="180" required/>
            </div>
            <div className="db-field">
              <label>Duration (minutes)</label>
              <input type="number" value={form.duration_min} onChange={e=>setForm(f=>({...f,duration_min:e.target.value}))} placeholder="240"/>
            </div>
            <div className="db-field" style={{gridColumn:'1/-1'}}>
              <label>Description (optional)</label>
              <input value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="Short description of the service"/>
            </div>
            <button type="submit" className="db-btn db-btn-primary" style={{gridColumn:'1/-1'}} disabled={loading}>
...
function Settings({ workspace, toast, refetch }) {
  ...
  async function save(e) {
    ...
    try {
      ...
      if (error) { toast('Error saving. Try again.'); }
