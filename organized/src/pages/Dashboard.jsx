...
// ── PRODUCTS ──────────────────────────────────────────────────────────────────
// Shared upload helper — used by both add form and edit modal
async function uploadProductImages(files, workspaceId) {
  return Promise.all(files.map(async (file, i) => {
    ...
    if (error) return { preview, url: null }
    const { data: urlData } = supabase.storage.from('product-images').getPublicUrl(path)
    return { preview, url: urlData?.publicUrl || null }
  }))
}
      ...
      {pendingImgs.length > 0 && (
        <div style={{display:'flex',gap:'.45rem',flexWrap:'wrap',marginTop:'.65rem'}}>
          {pendingImgs.map((img, i) => (
            <div key={i} style={{position:'relative',width:68,height:68,borderRadius:9,overflow:'hidden',border:'1px solid var(--border)',background:'var(--bg)',flexShrink:0}}>
              <img src={img.preview || img.url} alt="" style={{width:'100%',height:'100%',objectFit:'cover',opacity:img.url?1:.5,transition:'opacity .2s'}}/>
              {!img.url && <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'.55rem',color:'var(--ink-3)'}}>↑</div>}
      ...
      <div className="grid-3">
        {data.length===0
          ...
          :data.map(p=>{
            const imgs = p.images||[]
            return (
              <div key={p.id} className="prod-card" onClick={()=>setEditProduct(p)} style={{cursor:'pointer',position:'relative'}}>
                <div className="prod-img" style={{position:'relative',overflow:'hidden',background:'var(--bg)'}}>
                  {imgs.length>0
                    ?<img src={imgs[0]} alt={p.name} style={{width:'100%',height:'100%',objectFit:'cover',transition:'transform .3s'}}
                  ...
                  <div style={{position:'absolute',top:6,right:6,background:'rgba(0,0,0,.4)',color:'#fff',fontSize:'.6rem',padding:'2px 7px',borderRadius:20,opacity:0,transition:'opacity .15s'}}
                    onMouseEnter={e=>e.currentTarget.style.opacity=1} className="edit-hint">✏️ Edit</div>
                </div>
