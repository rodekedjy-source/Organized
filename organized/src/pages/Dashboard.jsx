...
export default function Dashboard() {
  ...
  function setTheme(t) { setThemeState(t); localStorage.setItem('org-theme', t) }
  function showToast(msg) { setToast(msg); setTimeout(() => setToast(null), 3200) }
  function navigateTo(p) { setPageStack(s => [...s, page]); setPage(p); setSidebarOpen(false) }
  function goBack() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      setSession(session)
      if (session) fetchWorkspace(session)
    })
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setSession(session)
      if (session) fetchWorkspace(session)
      else { setWorkspace(null); setOwnerData(null) }
    })
    return () => subscription.unsubscribe()
  }, [])

  async function fetchWorkspace(sess) {
    const uid = sess?.user?.id
    if (!uid) return
    const [{ data: ws }, { data: ud }] = await Promise.all([
      supabase.from('workspaces').select('*').eq('owner_id', uid).maybeSingle(),
  ...
  const navItems = [
    ...
    { key: 'appointments',  label: t(lang,'nav_appointments'),  icon: I.cal   },
  ...
  const initials = (ownerData?.full_name || session?.user?.email || 'U').charAt(0).toUpperCase()
    ...
    <div className={`app${theme === 'dark' ? ' dark' : ''}`}>
      ...
      <div className={`sidebar${sidebarOpen ? ' open' : ''}`}>
        <div className="sidebar-head">
          <div className="sidebar-logo">Organized<span style={{color:'var(--gold)'}}>.</span></div>
        ...
        <nav className="sidebar-nav">
          {navItems.map(n => (
            <button key={n.key}
              ...
              onClick={() => { setPage(n.key); setPageStack([]); setSidebarOpen(false) }}>
        ...
        <button className="sidebar-signout" onClick={handleSignOut}>
          <span className="sidebar-icon">{I.bell}</span>
          {t(lang,'nav_signout')}
        </button>
      </div>
      ...
      <header className="topbar">
        <div className="topbar-left">
          ...
          <div className="topbar-logo">Organized<span style={{color:'var(--gold)'}}>.</span></div>
        </div>
        <div className="view-toggle">
          <button className={activeView === 'dash' ? 'active' : ''} onClick={() => setActiveView('dash')}>Dash</button>
          <button className={activeView === 'client' ? 'active' : ''} onClick={() => setActiveView('client')}>Client</button>
        </div>
        <div className="topbar-avatar" onClick={() => { setPage('settings'); setPageStack([]); setSidebarOpen(false) }}>{initials}</div>
      </header>
      ...
      <main className="main">
        {activeView === 'client' ? (
          ...
          <>
            {page === 'overview'     && <Overview     workspace={workspace} session={session} ownerData={ownerData} toast={showToast} setPage={p=>{setPage(p);setPageStack(s=>[...s,'overview'])}} refetchWorkspace={()=>fetchWorkspace(session)} lang={lang}/>}
            ...
            {page === 'settings'     && <Settings     workspace={workspace} toast={showToast} refetch={()=>fetchWorkspace(session)} theme={theme} setTheme={setTheme} session={session} ownerData={ownerData} lang={lang}/>}
          </>
        )}
      </main>
    </div>
...
:root {
  --gold:#c9a96e;--gold-lt:rgba(201,169,110,.1);--gold-dim:rgba(201,169,110,.3);
  --ink:#1a1814;--ink-2:#2e2b27;--ink-3:#7a7774;
  --bg:#f5f3ef;--surface:#ffffff;--border:#ede9e3;--border-2:#d9d4cc;
  --green:#2e7d52;--red:#c0392b;--orange:#b45309;
}
[data-theme="dark"] {
  --ink:#f0ede8;--ink-2:#d0cdc8;--ink-3:#888480;
  --bg:#141210;--surface:#1e1c19;--border:#2a2824;--border-2:#3a3733;
}
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;background:var(--bg);color:var(--ink);-webkit-font-smoothing:antialiased}
.app{min-height:100vh;background:var(--bg)}

/* TOPBAR */
.topbar{position:sticky;top:0;z-index:90;background:var(--surface);border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;padding:0 1.1rem;height:54px;gap:.75rem}
.topbar-left{display:flex;align-items:center;gap:.5rem;flex:0 0 auto}
.topbar-logo{font-family:'Playfair Display',serif;font-size:1.1rem;font-weight:500;color:var(--ink);white-space:nowrap}
.hamburger{background:none;border:none;cursor:pointer;padding:.3rem;display:flex;flex-direction:column;gap:4px}
.hamburger span{display:block;width:20px;height:2px;background:var(--ink);border-radius:2px;transition:.2s}
.back-btn{background:var(--bg);border:1px solid var(--border);border-radius:8px;width:30px;height:30px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--ink-3);flex-shrink:0}
.view-toggle{display:flex;background:var(--bg);border:1px solid var(--border);border-radius:10px;overflow:hidden;flex-shrink:0}
.view-toggle button{padding:.3rem .85rem;border:none;background:none;font-family:inherit;font-size:.8rem;color:var(--ink-3);cursor:pointer;transition:.15s;white-space:nowrap}
.view-toggle button.active{background:var(--surface);color:var(--ink);font-weight:500;box-shadow:0 1px 4px rgba(0,0,0,.08)}
.topbar-avatar{width:34px;height:34px;border-radius:50%;background:var(--gold-lt);border:1.5px solid var(--gold);display:flex;align-items:center;justify-content:center;font-size:.8rem;font-weight:600;color:var(--gold);cursor:pointer;flex-shrink:0}

/* SIDEBAR */
.sidebar-overlay{position:fixed;inset:0;background:rgba(0,0,0,.35);z-index:99;backdrop-filter:blur(2px)}
.sidebar{position:fixed;top:0;left:0;bottom:0;width:260px;background:var(--surface);border-right:1px solid var(--border);z-index:100;display:flex;flex-direction:column;transform:translateX(-100%);transition:transform .28s cubic-bezier(.4,0,.2,1)}
...
.sidebar-logo{font-family:'Playfair Display',serif;font-size:1.1rem;font-weight:500;color:var(--ink)}
.sidebar-close{background:none;border:none;font-size:1.3rem;cursor:pointer;color:var(--ink-3);padding:.2rem}
.sidebar-nav{flex:1;overflow-y:auto;padding:.65rem 0}
.sidebar-item{display:flex;align-items:center;gap:.75rem;width:100%;padding:.72rem 1.25rem;border:none;background:none;font-family:inherit;font-size:.88rem;color:var(--ink-3);cursor:pointer;text-align:left;transition:.14s;border-radius:0}
.sidebar-item:hover{background:var(--bg);color:var(--ink)}
.sidebar-item.active{background:var(--gold-lt);color:var(--gold);font-weight:500}
.sidebar-icon{width:16px;height:16px;display:flex;align-items:center;justify-content:center;flex-shrink:0;opacity:.7}
.sidebar-signout{display:flex;align-items:center;gap:.75rem;width:100%;padding:.85rem 1.25rem;border:none;border-top:1px solid var(--border);background:none;font-family:inherit;font-size:.85rem;color:var(--ink-3);cursor:pointer}
...
.card-sub{font-size:.75rem;color:var(--ink-3);margin-top:1px}
.card-body{padding:1.25rem 1.4rem}

/* PAGE HEAD */
.page-head{margin-bottom:1.25rem}
.page-title{font-family:'Playfair Display',serif;font-size:2rem;color:var(--ink);font-weight:400;line-height:1.15}
.page-sub{font-size:.82rem;color:var(--ink-3);margin-top:.3rem}
.head-actions{display:flex;gap:.5rem;margin-top:.85rem;flex-wrap:wrap}

/* BUTTONS */
.btn{display:inline-flex;align-items:center;gap:.4rem;padding:.5rem 1rem;border-radius:9px;border:none;font-family:inherit;font-size:.82rem;font-weight:500;cursor:pointer;transition:all .15s;white-space:nowrap}
.btn-primary{background:var(--ink);color:#fff}.btn-primary:hover{opacity:.88}
.btn-primary:disabled{opacity:.5;cursor:default}
.btn-secondary{background:var(--surface);color:var(--ink);border:1px solid var(--border-2)}.btn-secondary:hover{background:var(--bg)}
.btn-sm{padding:.38rem .85rem;font-size:.78rem}
.btn-xs{padding:.28rem .65rem;font-size:.72rem;border-radius:7px}

/* NEXT UP BANNER */
.next-up-banner{background:var(--ink);border-radius:14px;padding:1.1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;justify-content:space-between;gap:.75rem}
.next-up-left{display:flex;align-items:center;gap:.85rem;flex:1;min-width:0}
.next-up-icon{width:34px;height:34px;border-radius:50%;background:rgba(201,169,110,.15);border:1px solid rgba(201,169,110,.3);display:flex;align-items:center;justify-content:center;color:var(--gold);flex-shrink:0}
.next-up-label{font-size:.68rem;color:rgba(255,255,255,.5);font-weight:600;text-transform:uppercase;letter-spacing:.06em;margin-bottom:.2rem}
.next-up-name{font-family:'Playfair Display',serif;font-size:1rem;color:#fff;font-weight:500;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.next-up-right{text-align:right;flex-shrink:0}
.next-up-time{font-size:1rem;font-weight:600;color:#fff}
.next-up-amount{font-size:.75rem;color:var(--gold);font-weight:500}
...
.stat-card{background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:1.1rem 1.25rem;flex:0 0 160px;position:relative}
.stat-card-btn{cursor:pointer;text-align:left;transition:transform .15s,box-shadow .15s}
.stat-card-btn:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(0,0,0,.08)}
.stat-label{font-size:.66rem;font-weight:600;color:var(--ink-3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:.5rem}
.stat-value{font-family:'Playfair Display',serif;font-size:1.75rem;color:var(--ink);font-weight:400;margin-bottom:.35rem;line-height:1}
.stat-delta{font-size:.72rem;font-weight:500}
.delta-up{color:var(--green)}
.delta-down{color:var(--red)}
.stat-arrow{position:absolute;top:1rem;right:1rem;font-size:.8rem;color:var(--border-2)}

/* COACH */
.coach-slider{background:var(--surface);border-radius:14px;border:1px solid var(--border);padding:1.1rem 1.4rem;margin-bottom:1.25rem}
.coach-slider-label{font-size:.66rem;font-weight:700;color:var(--gold);text-transform:uppercase;letter-spacing:.1em;margin-bottom:.65rem}
.coach-slider-body{display:flex;align-items:flex-start;gap:.75rem;min-height:2.4rem;transition:opacity .3s}
.coach-fade-out{opacity:0}
.coach-slider-icon{font-size:1.1rem;flex-shrink:0;margin-top:.05rem}
.coach-slider-text{font-size:.84rem;color:var(--ink-2);line-height:1.55}
.coach-slider-footer{display:flex;align-items:center;gap:.75rem;margin-top:.85rem}
.coach-slider-dots{display:flex;gap:.35rem}
.coach-dot{width:6px;height:6px;border-radius:50%;background:var(--border-2);border:none;cursor:pointer;padding:0;transition:.2s}
.coach-dot-active{background:var(--gold);width:18px;border-radius:3px}
...
/* BAR CHART */
.bar-chart{display:flex;align-items:flex-end;gap:4px;height:70px;margin-bottom:.5rem}
.bar{flex:1;border-radius:4px 4px 0 0;background:var(--border);transition:height .4s cubic-bezier(.4,0,.2,1)}
...
/* GOAL */
.goal-track{height:6px;background:var(--border);border-radius:3px;overflow:hidden}
.goal-fill{height:100%;background:linear-gradient(90deg,var(--gold),#e8c77a);border-radius:3px;transition:width .8s cubic-bezier(.4,0,.2,1)}
.goal-hint{font-size:.74rem;color:var(--ink-3);margin-top:.6rem;line-height:1.5}

/* TOP SERVICE */
.top-badge{display:flex;align-items:center;gap:.25rem;font-size:.7rem;color:var(--gold);background:var(--gold-lt);border:1px solid var(--gold-dim);padding:.2rem .6rem;border-radius:20px;font-weight:600}
.top-track{height:5px;background:var(--border);border-radius:3px;overflow:hidden}
.top-fill{height:100%;background:var(--gold);border-radius:3px}

/* REVENUE PANEL */
.rev-overlay{position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:150;display:flex;align-items:flex-end;justify-content:center;backdrop-filter:blur(4px)}
.rev-panel{background:var(--surface);border-radius:20px 20px 0 0;width:100%;max-width:680px;padding:1.75rem 1.5rem 3rem;max-height:80vh;overflow-y:auto}
.rev-panel-head{display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:1.25rem}
.rev-panel-title{font-family:'Playfair Display',serif;font-size:1.4rem;color:var(--ink);font-weight:400}
.rev-panel-sub{font-size:.8rem;color:var(--ink-3);margin-top:.2rem;text-transform:capitalize}
.rev-close{background:var(--bg);border:none;width:30px;height:30px;border-radius:50%;cursor:pointer;color:var(--ink-3);font-size:.9rem;display:flex;align-items:center;justify-content:center}
.rev-total{font-family:'Playfair Display',serif;font-size:2.2rem;color:var(--ink);font-weight:400;margin-bottom:1.5rem}
.rev-chart-wrap{margin-bottom:1rem;position:relative}
.rev-axis{position:relative;height:18px;font-size:.6rem;color:var(--ink-3)}
.rev-pills{display:flex;gap:.65rem;flex-wrap:wrap;margin-bottom:1.25rem}
.rev-pill{display:flex;align-items:center;gap:.6rem;background:var(--bg);border-radius:10px;padding:.6rem .85rem;flex:1;min-width:120px}
.rev-pill-icon{width:26px;height:26px;border-radius:50%;background:rgba(46,125,82,.1);color:var(--green);display:flex;align-items:center;justify-content:center;font-size:.88rem;flex-shrink:0}
.rev-pill-avg{background:rgba(201,169,110,.1);color:var(--gold)}
.rev-pill-low{background:rgba(192,57,43,.08);color:var(--red)}
.rev-pill-label{font-size:.66rem;color:var(--ink-3);font-weight:600;text-transform:uppercase;letter-spacing:.05em}
.rev-pill-val{font-size:.88rem;font-weight:600;color:var(--ink);margin-top:.1rem}
.rev-narrative{font-size:.82rem;color:var(--ink-3);line-height:1.65;padding:.85rem 1rem;background:var(--bg);border-radius:10px}
...
@media(min-width:420px){.grid-3{grid-template-columns:repeat(2,1fr)}}

/* CALENDAR */
.cal-nav-btn{background:var(--bg);border:1px solid var(--border);border-radius:8px;width:28px;height:28px;cursor:pointer;font-size:1.1rem;display:flex;align-items:center;justify-content:center;color:var(--ink-3)}
.cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:2px}
.cal-dname{text-align:center;font-size:.6rem;font-weight:600;color:var(--ink-3);padding:3px 0}
.cal-d{text-align:center;font-size:.8rem;padding:6px 2px;border-radius:6px;color:var(--ink);position:relative;user-select:none;transition:background .12s}
.cal-d:not(.cal-empty):hover{background:var(--bg)}
.cal-d.today{background:var(--ink);color:#fff;font-weight:600}
.cal-d.cal-blocked{color:var(--red)}
.cal-d.cal-has-appt{font-weight:600}
...
.tbl th{text-align:left;font-size:.65rem;font-weight:600;color:var(--ink-3);text-transform:uppercase;letter-spacing:.06em;padding:.6rem 1.25rem;border-bottom:1px solid var(--border)}
.tbl td{padding:.75rem 1.25rem;border-bottom:1px solid var(--border);color:var(--ink)}
.tbl tr:last-child td{border-bottom:none}
.tbl-name{font-weight:500}
.tbl-amount{font-family:'Playfair Display',serif;font-size:.95rem}

/* BADGES */
.badge{display:inline-flex;align-items:center;gap:.3rem;font-size:.7rem;font-weight:600;padding:.2rem .6rem;border-radius:20px}
.badge-confirmed{background:rgba(46,125,82,.08);color:var(--green)}
.badge-pending{background:rgba(180,83,9,.08);color:var(--orange)}
.badge-cancelled{background:rgba(192,57,43,.08);color:var(--red)}
.badge-low{background:rgba(192,57,43,.08);color:var(--red)}
.badge-vip{background:var(--gold-lt);color:var(--gold)}
.badge-new{background:rgba(59,130,246,.08);color:#3b82f6}
...
.empty-icon{width:48px;height:48px;margin:0 auto 1rem;color:var(--border-2)}
.empty-title{font-size:.95rem;font-weight:600;color:var(--ink);margin-bottom:.35rem}
.empty-sub{font-size:.8rem;color:var(--ink-3);max-width:260px;line-height:1.55}

/* FIELD */
.field{display:flex;flex-direction:column;gap:.3rem}
.field label{font-size:.72rem;font-weight:600;color:var(--ink-3);text-transform:uppercase;letter-spacing:.06em}
.field input,.field textarea,.field select{padding:.55rem .85rem;border:1px solid var(--border-2);border-radius:9px;font-size:.88rem;font-family:inherit;color:var(--ink);background:var(--surface);outline:none;transition:border .15s;width:100%}
.field input:focus,.field textarea:focus,.field select:focus{border-color:var(--gold)}

/* MILESTONE */
.milestone-banner{background:linear-gradient(135deg,var(--ink),#2a2520);border-radius:12px;padding:1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;gap:.85rem;animation:milestoneIn .35s ease}
.milestone-icon{font-size:1.4rem;flex-shrink:0}
.milestone-text{font-size:.85rem;color:rgba(255,255,255,.82);line-height:1.5}
@keyframes milestoneIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}

/* PRODUCTS */
.prod-card{background:var(--surface);border-radius:13px;border:1px solid var(--border);overflow:hidden;transition:box-shadow .2s}
.prod-card:hover{box-shadow:0 6px 22px rgba(0,0,0,.09)}
.prod-img{height:170px}
.prod-body{padding:.85rem 1rem}
.prod-name{font-weight:600;font-size:.88rem;color:var(--ink);margin-bottom:.2rem}
.prod-price{font-family:'Playfair Display',serif;font-size:1rem;color:var(--gold);margin-bottom:.4rem}

/* FORMATIONS */
.formation-row{display:flex;align-items:center;gap:1rem;padding:1rem 1.4rem;border-bottom:1px solid var(--border)}
.formation-icon-block{width:36px;height:36px;border-radius:9px;background:var(--gold-lt);display:flex;align-items:center;justify-content:center;color:var(--gold);flex-shrink:0}
.formation-info{flex:1;min-width:0}
.formation-name{font-weight:600;font-size:.88rem;color:var(--ink)}
.formation-desc{font-size:.75rem;color:var(--ink-3);margin-top:.15rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.formation-meta{font-size:.7rem;color:var(--ink-3);background:var(--bg);border:1px solid var(--border);border-radius:6px;padding:2px 8px;display:inline-flex}
.formation-price{font-family:'Playfair Display',serif;font-size:1.05rem;color:var(--ink);flex-shrink:0}

/* SETTINGS */
.settings-back-btn{display:inline-flex;align-items:center;gap:.4rem;background:none;border:none;font-family:inherit;font-size:.8rem;color:var(--ink-3);cursor:pointer;padding:.2rem 0;margin-bottom:.25rem}
.settings-section-row{display:flex;align-items:center;justify-content:space-between;padding:1rem 1.4rem;cursor:pointer;transition:background .14s}
.settings-section-row:hover{background:var(--bg)}
.settings-section-label{font-size:.88rem;font-weight:500;color:var(--ink)}
.settings-section-sub{font-size:.75rem;color:var(--ink-3);margin-top:.15rem}
.settings-row{display:flex;align-items:center;justify-content:space-between;padding:.65rem 0}
.settings-row-label{font-size:.85rem;color:var(--ink)}
.toggle-wrap{position:relative;display:inline-flex;cursor:pointer}
.toggle-wrap input{opacity:0;width:0;height:0;position:absolute}
.toggle-track{width:42px;height:24px;background:var(--border-2);border-radius:12px;transition:.2s}
.toggle-thumb{position:absolute;top:2px;left:2px;width:20px;height:20px;background:#fff;border-radius:50%;box-shadow:0 1px 4px rgba(0,0,0,.2);transition:.2s}
...
.lang-row{display:flex;align-items:center;justify-content:space-between;padding:.85rem 1rem;border-radius:10px;border:1.5px solid var(--border);cursor:pointer;transition:.14s}
.lang-row:hover{border-color:var(--gold)}
.lang-active{border-color:var(--gold);background:var(--gold-lt)}
.lang-label{font-size:.88rem;font-weight:500;color:var(--ink)}
.lang-sub{font-size:.72rem;color:var(--ink-3);margin-top:.1rem}
.lang-check-circle{width:22px;height:22px;border-radius:50%;border:2px solid var(--border-2);display:flex;align-items:center;justify-content:center;transition:.14s;flex-shrink:0}
.lang-check-active{background:var(--gold);border-color:var(--gold)}
.theme-options{display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem}
.theme-option{border:2px solid var(--border);border-radius:12px;padding:1rem;cursor:pointer;transition:.14s}
.theme-option.selected{border-color:var(--gold)}
.theme-preview{height:60px;border-radius:8px;overflow:hidden;display:flex;gap:4px;background:var(--bg);padding:6px;margin-bottom:.5rem}
.tp-ls{width:30px;background:#e5e0d8;border-radius:4px}.tp-lm{flex:1;display:flex;flex-direction:column;gap:3px}.tp-lb{height:8px;background:#d5d0c8;border-radius:3px}
.tp-ds{width:30px;background:#1e1c19;border-radius:4px}.tp-dm{flex:1;display:flex;flex-direction:column;gap:3px}.tp-db{height:8px;background:#2a2824;border-radius:3px}
.theme-label{font-size:.82rem;font-weight:500;color:var(--ink);display:flex;align-items:center;justify-content:space-between}
.theme-check{width:18px;height:18px;background:var(--gold);border-radius:50%;display:flex;align-items:center;justify-content:center}

/* AVAILABILITY */
.avail-time{padding:.4rem .55rem;border:1px solid var(--border-2);border-radius:8px;font-size:.8rem;font-family:inherit;color:var(--ink);background:var(--surface);outline:none;transition:border .15s}
.avail-time:focus{border-color:var(--gold)}

/* TOAST */
.toast{position:fixed;bottom:1.5rem;left:50%;transform:translateX(-50%);background:var(--ink);color:#fff;padding:.65rem 1.4rem;border-radius:30px;font-size:.82rem;z-index:999;animation:toastIn .3s ease;white-space:nowrap;max-width:90vw;text-align:center}
@keyframes toastIn{from{opacity:0;transform:translateX(-50%) translateY(12px)}to{opacity:1;transform:translateX(-50%) translateY(0)}}

/* CLIENT PAGE */
.cp-topbar{position:sticky;top:0;background:#fff;border-bottom:1px solid #ede9e3;display:flex;align-items:center;justify-content:space-between;padding:.85rem 1.25rem;z-index:10}
.cp-logo{font-family:'Playfair Display',serif;font-size:1.1rem;color:#1a1814}
.cp-ghost{background:none;border:none;font-family:inherit;font-size:.82rem;color:#7a7774;cursor:pointer}
.cp-hero{padding:2.5rem 1.5rem 1.5rem;text-align:center;background:linear-gradient(180deg,#1a1814 0%,#2e2b27 100%)}
.cp-av{width:72px;height:72px;border-radius:50%;background:var(--gold-lt);border:2px solid var(--gold);display:flex;align-items:center;justify-content:center;font-size:1.6rem;font-weight:600;color:var(--gold);margin:0 auto 1rem}
.cp-name{font-family:'Playfair Display',serif;font-size:1.65rem;color:#fff;font-weight:400;margin-bottom:.3rem}
.cp-bio{font-size:.82rem;color:rgba(255,255,255,.55)}
.cp-stats{display:flex;justify-content:center;gap:2.5rem;margin-top:1.25rem}
.cp-sval{font-family:'Playfair Display',serif;font-size:1.4rem;color:#fff}
.cp-slbl{font-size:.68rem;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.08em;margin-top:.15rem}
.cp-nav{display:flex;border-bottom:1px solid #ede9e3;padding:0 1rem}
.cp-tab{padding:.85rem 1.25rem;font-size:.85rem;color:#7a7774;cursor:pointer;border-bottom:2px solid transparent;transition:.15s}
.cp-tab.active{color:#1a1814;border-bottom-color:#c9a96e;font-weight:500}
.cp-body{padding:1.5rem 1.25rem;max-width:680px;margin:0 auto}
.cp-stitle{font-family:'Playfair Display',serif;font-size:1.15rem;margin-bottom:1rem;color:#1a1814}
.cp-footer{text-align:center;padding:2rem;font-size:.75rem;color:#b0aba4;border-top:1px solid #ede9e3}
.svc-list{display:flex;flex-direction:column;gap:.65rem}
.svc-row{display:flex;align-items:center;gap:1rem;padding:1rem 1.25rem;background:#fff;border:1px solid #ede9e3;border-radius:12px;cursor:pointer;transition:.15s}
.svc-row:hover{box-shadow:0 4px 16px rgba(0,0,0,.06)}
.svc-bar{width:3px;height:36px;background:var(--gold);border-radius:2px;flex-shrink:0}
.svc-info{flex:1}
.svc-name{font-weight:500;color:#1a1814;font-size:.88rem}
.svc-dur{font-size:.72rem;color:#7a7774;margin-top:.15rem}
.svc-price{font-family:'Playfair Display',serif;font-size:1rem;color:#1a1814}
.bk-btn{background:#1a1814;color:#fff;border:none;border-radius:8px;padding:.4rem .9rem;font-size:.78rem;cursor:pointer;font-family:inherit}
.shop-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:1rem}
.shop-card{background:#fff;border-radius:12px;border:1px solid #ede9e3;overflow:hidden}
.shop-img{height:130px;background:#f5f3ef;display:flex;align-items:center;justify-content:center;font-size:.7rem;color:#b0aba4}
.shop-body{padding:.85rem}
.shop-name{font-weight:500;color:#1a1814;font-size:.85rem;margin-bottom:.2rem}
.shop-price{font-family:'Playfair Display',serif;font-size:.95rem;color:#1a1814;margin-bottom:.55rem}
.shop-btn{width:100%;background:#1a1814;color:#fff;border:none;border-radius:8px;padding:.4rem;font-size:.78rem;cursor:pointer;font-family:inherit}
.fm-card{display:flex;align-items:center;gap:1.1rem;padding:1.1rem 1.25rem;background:#fff;border-radius:12px;border:1px solid #ede9e3;margin-bottom:.65rem}
.fm-idx{font-family:'Playfair Display',serif;font-size:1.5rem;color:#d9d4cc;flex-shrink:0;width:32px}
.fm-info{flex:1}
.fm-name{font-weight:500;color:#1a1814;font-size:.88rem}
.fm-desc{font-size:.75rem;color:#7a7774;margin-top:.15rem}
.fm-tags{display:flex;gap:.4rem;margin-top:.5rem;flex-wrap:wrap}
.fm-tags span{background:#f5f3ef;border:1px solid #ede9e3;border-radius:6px;padding:2px 8px;font-size:.68rem;color:#7a7774}
.fm-price{font-family:'Playfair Display',serif;font-size:1.05rem;color:#1a1814;flex-shrink:0}
...
.modal{background:#fff;border-radius:20px 20px 0 0;padding:1.75rem 1.5rem 3rem;width:100%;max-width:480px;display:flex;flex-direction:column;gap:1rem}
