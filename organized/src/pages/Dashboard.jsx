...
<div style={{fontSize:’.8rem’,color:‘var(–ink-3)’,marginBottom:‘1rem’}}>{count} booking{count>1?‘s’:’’} · {fmtRev(rev)} earned</div>
...
{highest&&<div className="rev-pill"><span className="rev-pill-icon">↑</span><div><div className="rev-pill-label">Meilleur jour</div><div className="rev-pill-val">{fmtRev(highest.total)} · {highest.day} {monthName.split(’ ‘)[0]}</div></div></div>}
...
{lowest&&lowest.day!==highest?.day&&<div className="rev-pill"><span className="rev-pill-icon rev-pill-low">↓</span><div><div className="rev-pill-label">Jour le plus bas</div><div className="rev-pill-val">{fmtRev(lowest.total)} · {lowest.day} {monthName.split(’ ’)[0]}</div></div></div>}
</div>
)}
...
{existingImgs.map((img,i)=>(<div key={i} style={{position:‘relative’,width:68,height:68,borderRadius:9,overflow:‘hidden’,border:‘1px solid var(–border)’,flexShrink:0}}><img src={img.url} alt=”” style={{width:‘100%’,height:‘100%’,objectFit:‘cover’}}/><button type=“button” onClick={()=>{setExistingImgs(prev=>prev.filter((*,idx)=>idx!==i));if(activeIdx>=allImages.length-1)setActiveIdx(0)}} style={{position:‘absolute’,top:2,right:2,background:‘rgba(192,57,43,.85)’,border:‘none’,color:’#fff’,width:18,height:18,borderRadius:‘50%’,cursor:‘pointer’,fontSize:’.58rem’,display:‘flex’,alignItems:‘center’,justifyContent:‘center’}}>✕</button></div>))}
</div>
</div>
)}
<div>
