...
// ── NEXT UP BANNER ────────────────────────────────────────────────────────────
function NextUpBanner({ appts, workspace, onReloaded, toast }) {
  const now = new Date()
  const next = appts
    ...
    <>
      <div className="next-up-banner">
        <div className="next-up-left">
          ...
          <div>
            <div className="next-up-label">Next appointment {countdown}</div>
...
function MonthlyGoal({ appts, workspace, refetchWorkspace }) {
  ...
  useEffect(() => {
    const t = setTimeout(() => setDisplayPct(pctVal), 120)
...
// ── AI COACH ─────────────────────────────────────────────────────────────────
function AiCoach({ appts, stats, workspace }) {
  const insights = []
  const now = new Date()
  ...
  if (pending.length > 0)
    insights.push({ key:'pending', text:`You have ${pending.length} unconfirmed booking${pending.length>1?'s':''} pending. Confirming ${pending.length>1?'them':'it'} secures your schedule and your client's commitment.` })

  // Today's appointments with no reminder sent
  const todayStr = now.toISOString().split('T')[0]
  const todayConfirmed = appts.filter(a => a.scheduled_at?.startsWith(todayStr) && a.status==='confirmed')
  const noReminder = todayConfirmed.filter(a => !a.reminder_sent_at)
  if (noReminder.length > 0)
    insights.push({ key:'reminder', text:`${noReminder.length} client${noReminder.length>1?'s have':' has'} not received a reminder for today's appointment${noReminder.length>1?'s':''}. A quick message reduces no-shows significantly.` })

  // Goal gap analysis
  const goal = workspace?.monthly_revenue_goal || 3000
  const mRev = monthRevenue(appts)
  const pct = Math.round((mRev/goal)*100)
  const remaining = Math.max(goal - mRev, 0)
  const confirmedWithAmt = appts.filter(a => a.status==='confirmed' && Number(a.amount)>0)
  const avg = confirmedWithAmt.length ? mRev / confirmedWithAmt.length : 0
  if (pct < 50 && avg > 0) {
    const needed = Math.ceil(remaining / avg)
    insights.push({ key:'goal', text:`You're at ${pct}% of your ${new Date().toLocaleDateString('en-US',{month:'long'})} goal. ${needed} more appointment${needed>1?'s':''} at your current average would close the gap.` })
  }
  ...
  if (upcoming.length === 0 && insights.length < 2)
    insights.push({ key:'link', text:`Your next 3 days are open. Sharing your booking link on Instagram today could fill those slots before the week ends.` })

  if (insights.length === 0) return null
  return (
    <div className="ai-coach">
      <div className="ai-coach-header">Coach</div>
      {insights.slice(0,3).map(ins=>(
        <div key={ins.key} className="ai-coach-row">
          <div className="ai-coach-line"/>
          <div className="ai-coach-text">{ins.text}</div>
        </div>
      ))}
    </div>
  )
...
// ── OVERVIEW ──────────────────────────────────────────────────────────────────
function Overview({ workspace, session, toast, setPage, refetchWorkspace }) {
  ...
  async function fetchData(){
    ...
    const[a,p,e]=await Promise.all([
      // FIX: join services(name) to get service name on each appointment
  ...
  const bookedDays=[...new Set(allAppts.map(a=>a.scheduled_at?new Date(a.scheduled_at).getDate():null).filter(Boolean))]
  ...
  return (
    <div>
      <div className="page-head">
        <div>
          <div className="page-title">{greet()}, {firstName(workspace,session)}</div>
        ...
        <div className="head-actions">
          <button className="btn btn-secondary btn-sm" onClick={()=>{navigator.clipboard?.writeText(`https://beorganized.io/${workspace?.slug||''}`);toast('Booking link copied!')}}>
            <span style={{width:14,height:14,display:'flex'}}>{I.link}</span> Copy link
          </button>
          <button className="btn btn-primary btn-sm" onClick={()=>toast('New appointment coming soon.')}>New appointment</button>
        </div>
      </div>

      <NextUpBanner appts={allAppts} workspace={workspace} onReloaded={fetchData} toast={toast}/>
      <MilestoneBanner appts={allAppts} stats={stats}/>
      <AiCoach appts={allAppts} stats={stats} workspace={workspace}/>
      ...
      <div className="grid-2" style={{marginBottom:'1.25rem'}}>
        <div className="card" style={{marginBottom:0}}>
          <div className="card-head"><div className="card-title">Revenue — this week</div></div>
          <div className="card-body"><WeekChart appts={allAppts}/></div>
        </div>
        {/* FIX: passes workspace + refetchWorkspace so goal persists in DB */}
      ...
      <div className="grid-2" style={{marginBottom:'1.25rem'}}>
        ...
        <div className="card" style={{marginBottom:0}}>
          <div className="card-head"><div className="card-title">Calendar — {new Date().toLocaleDateString('en-US',{month:'long',year:'numeric'})}</div></div>
          <div className="card-body"><MiniCal bookedDays={bookedDays}/></div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-head">
          ...
          <span className="badge badge-confirmed">{todayCount} confirmed</span>
        </div>
        {todayCount===0?(
          <div className="empty-state">
            <div className="empty-icon">{I.cal}</div>
            <div className="empty-title">Your day is open</div>
            <div className="empty-sub">Share your booking link to fill your schedule.</div>
            <button className="btn btn-primary btn-sm" style={{marginTop:'.75rem'}}
              onClick={()=>{navigator.clipboard?.writeText(`https://beorganized.io/${workspace?.slug||''}`);toast('Link copied!')}}>
              Copy booking link
            </button>
          </div>
        ):(
          <table className="tbl">
            ...
            <tbody>{appts.map(a=>(
              <tr key={a.id}>
                ...
                <td>{svcName(a)}</td>{/* FIX: uses joined service name */}
...
function Appointments({ workspace, toast }) {
  ...
  return (
    <div>
      <div className="page-head"><div><div className="page-title">Appointments</div><div className="page-sub">Manage your bookings</div></div></div>
...
function Services({ workspace, toast }) {
  ...
  return (
    <div>
      <div className="page-head">
        <div><div className="page-title">Services</div><div className="page-sub">What you offer — appears on your public profile</div></div>
        <button className="btn btn-primary" onClick={()=>setShowForm(s=>!s)}>{showForm?'Cancel':'+ Add service'}</button>
      </div>
      {showForm&&(
        <div className="card" style={{marginBottom:'1.25rem'}}>
      ...
      <div className="card">
        {data.length===0
          ?<div className="empty-state"><div className="empty-icon">{I.box}</div><div className="empty-title">No services yet</div><div className="empty-sub">Add your first service to start receiving bookings.</div></div>
          :<table className="tbl">
...
function Clients({ workspace }) {
  ...
  return (
    <div>
      <div className="page-head"><div><div className="page-title">Clients</div><div className="page-sub">Your client base</div></div></div>
      <div className="card">
        {loading?<div style={{padding:'2rem',color:'var(--ink-3)'}}>Loading...</div>
          :data.length===0?<div className="empty-state"><div className="empty-icon">{I.users}</div><div className="empty-title">No clients yet</div><div className="empty-sub">Clients appear here when they book with you.</div></div>
          :<table className="tbl">
...
// ── SETTINGS ──────────────────────────────────────────────────────────────────
function Settings({ workspace, toast, refetch, theme, setTheme, session, ownerData }) {
  const [section, setSection] = useState(null)

  const sections = [
    { key:'profile',    label:'My Profile',    sub:'Name, email address, password' },
    { key:'business',   label:'My Business',   sub:'Business info, contact, social media' },
    { key:'appearance', label:'Appearance',    sub:'Theme and display preferences' },
    { key:'language',   label:'Language',      sub:'English, Français, Español' },
  ]

  const BackBtn = ({ onClick }) => (
    <button className="settings-back-btn" onClick={onClick}>
  ...
  if (section === 'profile') return (
    <div>
      <div className="page-head"><div><BackBtn onClick={()=>setSection(null)}/><div className="page-title" style={{marginTop:'.5rem'}}>My Profile</div></div></div>
      <SettingsProfile session={session} ownerData={ownerData} toast={toast} refetch={refetch}/>
    </div>
  )
  if (section === 'business') return (
    <div>
      <div className="page-head"><div><BackBtn onClick={()=>setSection(null)}/><div className="page-title" style={{marginTop:'.5rem'}}>My Business</div></div>
        <button className="btn btn-secondary" onClick={async()=>{await supabase.from('workspaces').update({is_published:!workspace.is_published}).eq('id',workspace.id);toast(workspace.is_published?'Profile unpublished.':'Profile is now live!');refetch()}}>{workspace?.is_published?'Unpublish':'Publish profile'}</button>
      </div>
      <SettingsBusiness workspace={workspace} toast={toast} refetch={refetch}/>
    </div>
  )
  if (section === 'appearance') return (
    <div>
      <div className="page-head"><div><BackBtn onClick={()=>setSection(null)}/><div className="page-title" style={{marginTop:'.5rem'}}>Appearance</div></div></div>
      <SettingsAppearance theme={theme} setTheme={setTheme}/>
    </div>
  )
  if (section === 'language') return (
    <div>
      <div className="page-head"><div><BackBtn onClick={()=>setSection(null)}/><div className="page-title" style={{marginTop:'.5rem'}}>Language</div></div></div>
      <SettingsLanguage workspace={workspace} toast={toast}/>
    </div>
  )

  return (
    <div>
      <div className="page-head"><div><div className="page-title">Settings</div><div className="page-sub">Manage your account and preferences</div></div></div>
      <div className="card">
        {sections.map((s, i) => (
          <div key={s.key} className="settings-section-row" style={{borderBottom: i < sections.length-1 ? '1px solid var(--border)' : 'none'}}
...
function SettingsProfile({ session, ownerData, toast, refetch }) {
  ...
  async function saveProfile(e) {
    ...
    if (ownerData?.id) await supabase.from('users').update({full_name:form.full_name}).eq('id', ownerData.id)
    if (form.email !== session?.user?.email) await supabase.auth.updateUser({email: form.email})
    toast('Profile saved.'); setSaved(true); setLoading(false); if(refetch) refetch()
  }
  ...
  async function changePw(e) {
    ...
    else { toast('A confirmation link has been sent to your email.'); setPwForm({newPassword:''}) }
    setPwLoading(false)
  }
  ...
  return (
    <>
      <div className="card" style={{marginBottom:'1.25rem'}}>
        ...
        <form onSubmit={saveProfile} className="card-body" style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
          ...
          <div className="field"><label>Email address</label><input type="email" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} required/></div>
      ...
      <div className="card">
        ...
        <form onSubmit={changePw} className="card-body" style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
          <div className="field"><label>New password</label><input type="password" value={pwForm.newPassword} onChange={e=>setPwForm({newPassword:e.target.value})} placeholder="Minimum 8 characters" minLength={8} required/></div>
          <div style={{fontSize:'.76rem',color:'var(--ink-3)',background:'var(--bg)',padding:'.6rem .85rem',borderRadius:'8px',lineHeight:1.5}}>
            A confirmation link will be sent to your email before the change takes effect.
          </div>
          <button type="submit" className="btn btn-primary" style={{justifyContent:'center',padding:'.75rem'}} disabled={pwLoading}>
            {pwLoading?'Sending...':'Update password'}
          </button>
        </form>
      </div>
    </>
...
function SettingsBusiness({ workspace, toast, refetch }) {
  ...
  async function save(e) {
    ...
    if(error) toast(`Error: ${error.message}`)
    else { setSaved(true); toast('Business profile saved.'); refetch() }
    setLoading(false)
  }
      ...
      <form onSubmit={save} className="card-body" style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
        ...
        <div className="field"><label>Bio</label>
          <textarea value={form.bio} onChange={e=>setForm(f=>({...f,bio:e.target.value}))} rows={3}
            ...
            placeholder="Tell your clients about your work and experience"/>
        </div>
        ...
        <div style={{fontSize:'.7rem',fontWeight:600,color:'var(--ink-3)',textTransform:'uppercase',letterSpacing:'.08em'}}>Contact</div>
        <div className="field"><label>Business email</label><input type="email" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} placeholder="Can be the same as your personal email"/></div>
        <div className="field"><label>Phone</label><input value={form.phone} onChange={e=>setForm(f=>({...f,phone:e.target.value}))}/></div>
        <div style={{height:'1px',background:'var(--border)',margin:'.25rem 0'}}/>
        <div style={{fontSize:'.7rem',fontWeight:600,color:'var(--ink-3)',textTransform:'uppercase',letterSpacing:'.08em'}}>Social</div>
...
function SettingsAppearance({ theme, setTheme }) {
  ...
  const I_check = <svg viewBox="0 0 10 10" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="1.5,5 4,7.5 8.5,2.5"/></svg>
  return (
    <div className="card">
      <div className="card-head"><div className="card-title">Theme</div></div>
      <div className="card-body">
        <div style={{fontSize:'.82rem',color:'var(--ink-3)',marginBottom:'1rem'}}>Choose how Organized appears for you.</div>
        <div className="theme-options">
          <div className={`theme-option${!isDark?' selected':''}`} onClick={()=>setTheme('light')}>
            <div className="theme-preview"><div className="tp-ls"/><div className="tp-lm"><div className="tp-lb"/><div className="tp-lb"/></div></div>
            <div className="theme-label">Light {!isDark&&<div className="theme-check">{I_check}</div>}</div>
          </div>
          <div className={`theme-option${isDark?' selected':''}`} onClick={()=>setTheme('dark')}>
            <div className="theme-preview"><div className="tp-ds"/><div className="tp-dm"><div className="tp-db"/><div className="tp-db"/></div></div>
            <div className="theme-label">Dark {isDark&&<div className="theme-check">{I_check}</div>}</div>
          </div>
        </div>
...
function SettingsLanguage({ workspace, toast }) {
  ...
  return (
    <div className="card">
      <div className="card-head"><div className="card-title">Display language</div></div>
      <div style={{padding:'1rem 1.4rem',display:'flex',flexDirection:'column',gap:'.5rem'}}>
        ...
        <div style={{marginTop:'.75rem',fontSize:'.75rem',color:'var(--ink-3)',lineHeight:1.6,padding:'.6rem .85rem',background:'var(--bg)',borderRadius:'8px'}}>
          <span translate="no" style={{fontWeight:500}}>Organized.</span> remains <span translate="no" style={{fontWeight:500}}>Organized.</span> regardless of the language selected.
        </div>
      </div>
    </div>
  )
...
export default function Dashboard({ session }) {
  ...
  const navItems=[
    ...
    {key:'appointments',label:'Appointments', icon:I.cal,  count:pendingCount||null},
    {key:'services',    label:'Services',     icon:I.box},
    {key:'products',    label:'Products',     icon:I.box},
    {key:'formations',  label:'Formations',   icon:I.grad},
      ...
      {view==='dashboard'?(
        <div className="app-shell">
          ...
          <aside className={`sidebar${sidebarOpen?' open':''}`}>
            ...
            <div className="nav-item" onClick={handleSignOut}>Sign out</div>
          ...
          <main className="main">
            ...
            {page==='overview'    &&<Overview    workspace={workspace} session={session} toast={showToast} setPage={navigateTo} refetchWorkspace={fetchWorkspace}/>}
            ...
            {page==='clients'     &&<Clients     workspace={workspace}/>}
            {page==='settings'    &&<Settings    workspace={workspace} toast={showToast} refetch={fetchWorkspace} theme={theme} setTheme={setTheme} session={session} ownerData={ownerData}/>}
          </main>
        </div>
      ):(
...
.next-up-name{font-size:.88rem;font-weight:500;color:#fff;}
...
.settings-back-btn{display:inline-flex;align-items:center;gap:.4rem;background:none;border:none;cursor:pointer;font-size:.78rem;color:var(--ink-3);font-family:inherit;padding:0;transition:color .15s;margin-bottom:.25rem;}
.settings-back-btn:hover{color:var(--ink);}
.settings-back-btn svg{stroke:currentColor;}
.settings-section-row{display:flex;align-items:center;justify-content:space-between;padding:1.1rem 1.4rem;cursor:pointer;transition:background .12s;}
...
.lang-row{display:flex;align-items:center;justify-content:space-between;padding:.9rem 1.1rem;border-radius:10px;border:1.5px solid var(--border);background:var(--surface);cursor:pointer;transition:all .15s;}
...
.ai-coach{background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:1rem 1.4rem;margin-bottom:1rem;animation:coachIn .35s cubic-bezier(.34,1.2,.64,1);}
@keyframes coachIn{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}
.ai-coach-header{font-size:.68rem;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:var(--gold);margin-bottom:.75rem;}
.ai-coach-row{display:flex;align-items:flex-start;gap:.75rem;padding:.5rem 0;}
.ai-coach-row+.ai-coach-row{border-top:1px solid var(--border);}
.ai-coach-line{width:2px;min-height:36px;background:var(--border);border-radius:2px;flex-shrink:0;margin-top:.15rem;}
.ai-coach-text{font-size:.81rem;color:var(--ink-2);line-height:1.6;}
