          ...
          <div style={{background:'#fff',borderRadius:'14px',padding:'2rem',width:'420px',maxWidth:'90vw'}} onClick={e => e.stopPropagation()}>
            {booked ? (
              ...
              <form onSubmit={submitBooking}>
                ...
                {[['name','Full name','e.g. Amara Diallo','text'],['phone','Phone number','+1 (514) 555-0123','text'],['date','Preferred date','','date'],['time','Preferred time','','time'],['notes','Notes (optional)','Special requests...','text']].map(([key,label,placeholder,type]) => (
                  <div key={key} style={{marginBottom:'.85rem'}}>
                    ...
                    <input style={{width:'100%',padding:'.65rem .9rem',border:'1px solid #e4e0d8',borderRadius:'7px',fontSize:'.84rem',fontFamily:'inherit',color:'#0d0c0a',outline:'none'}} type={type} value={bookForm[key]} onChange={e=>setBookForm(f=>({...f,[key]:e.target.value}))} placeholder={placeholder} required={key==='name'||key==='date'||key==='time'}/>
                  </div>
                ))}
