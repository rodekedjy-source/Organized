...
const css = `
  ...
  .auth-input{width:100%;padding:.72rem 1rem;border:1px solid #e4e0d8;border-radius:8px;font-size:.88rem;font-family:inherit;color:#0d0c0a;background:#fff;outline:none;transition:border .15s,box-shadow .15s;}
...
export default function Auth({ onAuth }) {
  ...
  async function resendCode() {
    ...
    setError('') // clear and show success via UI state
  }
          ...
          <div className="auth-box">
            ...
            {mode === 'signup' && step === 1 && (
              <form onSubmit={submitStep1}>
                ...
                <div className="auth-field"><label>Email address</label><input className="auth-input" type="email" value={form.email} onChange={e=>set('email',e.target.value)} placeholder="you@example.com"/></div>
                <div className="auth-field"><label>Password</label><input className="auth-input" type="password" value={form.password} onChange={e=>set('password',e.target.value)} placeholder="At least 8 characters"/></div>
                ...
                <div className="auth-field">
                  ...
                  <div className="city-wrap" ref={cityRef}>
                    <input
                      ...
                      onChange={e=>{setCityQuery(e.target.value);set('city',e.target.value);setShowCities(true)}}
                    ...
                    {showCities && citySuggestions.length > 0 && (
                      <div className="city-dropdown">
                        {citySuggestions.map(c=>(
                          <div key={c} className="city-option" onMouseDown={()=>{set('city',c);setCityQuery(c);setShowCities(false)}}>
                            {c}
                          </div>
                        ))}
                      </div>
